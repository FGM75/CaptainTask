...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun getProductos(): List<Producto> {
        ...
        while (cursor.moveToNext()) {
            ...
            if (tituloIndex != -1 && descripcionIndex != -1 && contadorIndex != -1) {
                ...
                // Verificar si la cadena de URI de la imagen no es nula
                val imagenUriString = cursor.getString(imagenIndex)
                val imagenUri = if (imagenUriString != null) Uri.parse(imagenUriString) else null
